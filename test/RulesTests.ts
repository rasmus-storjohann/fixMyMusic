...
describe("Album format parser", () => {
        ...
        it("can parse performer", () => {
                ...
                expect(parsed).to.deep.equal({ form: "concerto", performer: "Brendel" });
        });
        it("can parse number", () => {
        ...
        it("can parse opus number and number within the opus", () => {
                ...
                expect(parsed).to.deep.equal({ form: "concerto", opus: 2, opus_number: 4 });
        });
        it("throws on opus number array having more than two elements", () => {
                var dto = { form: "concerto", opus: [1, 2, 4] };
                var json = JSON.stringify(dto);

                expect(() => {new RuleFactory().parseFormat(json)}).to.throw(
                                            Error,
        ...
        it("can parse major key", () => {
                ...
                expect(parsed).to.deep.equal({ form: "concerto", mode: "major", key: "A" });
        });
        it("can parse minor key", () => {
                ...
                expect(parsed).to.deep.equal({ form: "concerto", mode: "minor", key: "A" });
        });
